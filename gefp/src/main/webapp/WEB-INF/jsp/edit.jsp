			...
			<tr>
				...
				<td><select name="runway">
						<c:forEach items="${runway}" var="runway">
							<option value="${runway.id}" ${runway == runway_id ? 'selected="selected"' : ''}>${runway.description}</option>
						</c:forEach>
				</select></td>
			</tr>
			<tr>
				...
				<td><select name="stage">
						<c:forEach items="${stage}" var="stage">
							<option value="${stage.id}" ${stage == stage_id ? 'selected="selected"' : ''}>${stage.sname}</option>
						</c:forEach>
				</select></td>
			</tr>
			<tr>
				<td>Check Point:</td>
				<td><textarea id="textarea" name="description" style="width: 80%; height: 8em;" >${checkpoints.description}</textarea> 
			</tr>
			<tr>
